...
class Trainer(object):
    ...
    def train_epoch(self, epoch, loss_weights):
        ...
        for batch_idx, (data, target) in loop:
            ...
            loop.set_postfix(loss=total_sep_loss/(batch_idx+1))
    ...
    def test(model, dataloader, scaler, graph, logger, args):
        ...
        test_results = []
        mae, mape, rmse, smape = test_metrics(y_pred, y_true)
        logger.info("SPEED, MAE: {:.2f}, MAPE: {:.4f}%, RMSE: {:2.3f}, SMAPE {:2.3f}%".format(mae, mape*100, rmse, smape*100))
        test_results.append([mae, mape, rmse, smape])
